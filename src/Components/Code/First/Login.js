...
function Login() {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch('https://jocular-elf-62138c.netlify.app/api/login', {
        method: 'POST',
        headers: {
      ...
      if (data.userId) {
        ...
        const surveyResponse = await fetch(`https://jocular-elf-62138c.netlify.app/api/survey/responses/check/${data.userId}`);
