...
function LunchBox() {
  ...
  const handleConfirmSubscribe = async () => {
    try {
      const response = await fetch(`${process.env.BACKEND_URL}/api/lunchbox/subscribe/${userId}/${lunchbox_id}`, {
        method: "POST",
      });
      if (response.ok) {
  ...
  useEffect(() => {
    const fetchLunchbox = async () => {
      ...
      try {
        const response = await fetch(`${process.env.BACKEND_URL}/api/lunchbox/${lunchbox_id}`, {
          method: "GET",
          headers: {
