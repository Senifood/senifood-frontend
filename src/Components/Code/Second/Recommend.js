...
function Recommend() {
  ...
  const fetchDiet = async () => {
    ...
    const userId = localStorage.getItem('userId');
    try {
      const response = await fetch('https://senifood-backend-rocif.run.goorm.site/api/diet', {
        method: 'POST',
        headers: {
  ...
  const handleLikeClick = async () => {
    ...
    if (likeImage === LikeButton2) {
      ...
      try {
        const response = await fetch(`https://senifood-backend-rocif.run.goorm.site/api/diet/likes/${userId}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
