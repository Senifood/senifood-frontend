...
function Recommend() {
  ...
  const fetchUserName = async () => {
    ...
    try {
      const response = await fetch(`https://jocular-elf-62138c.netlify.app/api/user/${userId}`);
  ...
  const fetchDiet = async () => {
    ...
    const userId = localStorage.getItem('userId');
    try {
      const response = await fetch('https://jocular-elf-62138c.netlify.app/api/diet', {
        method: 'POST',
        headers: {
  ...
  const handleLikeClick = async () => {
    ...
    if (likeImage === LikeButton2) {
      ...
      try {
        const response = await fetch(`https://jocular-elf-62138c.netlify.app/api/diet/likes/${userId}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
